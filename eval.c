...
static enum eval_status macro_expand_once(struct lisp_val ast,
                                          struct lisp_env *env,
                                          struct lisp_val *result) {
  // Default case
  *result = ast;

  struct lisp_cons *call_ast = lisp_val_cast(LISP_CONS, ast);
  if (call_ast == NULL) {
    return EV_SUCCESS;
  }

  struct lisp_symbol *macro_sym = lisp_val_cast(LISP_SYMBOL, call_ast->car);
  if (macro_sym == NULL) {
    return EV_SUCCESS;
  }

  // Have to save the original AST in case the macro isn't expanded
  // (try_macro_expand saves the symbol and the arguments, but not the
  // container)
  gc_push_root(*result);
  struct eval_result res = try_macro_expand(macro_sym, call_ast->cdr, env);
  gc_pop_root_expect(*result);

  if (res.status == EV_TAIL_CALL) {
    // Handle the tail-call in this function so we don't get escape to eval
    *result = res.ast;
    return EV_SUCCESS;
  } else {
    return res.status;
...
static enum eval_status macro_expand_full(struct lisp_val ast,
                                          struct lisp_env *env,
                                          struct lisp_val *result) {
  while (true) {
    enum eval_status res = macro_expand_once(ast, env, result);
    if (res == EV_EXCEPTION) {
      return res;
    }

    if (lisp_val_identical(ast, *result)) {
      return EV_SUCCESS;
    } else {
      ast = *result;
      // Keep going
...
/**
 * Signal to stop macro expansion for primitives and quote.
 */
static bool should_stop_expansion(struct lisp_val ast) {
  struct lisp_cons *call_ast = lisp_val_cast(LISP_CONS, ast);
  if (call_ast == NULL) {
    // Primitive, no need to expand
    return true;
  }
  ...
  return macro_sym == SYMBOL_QUOTE;
}

static enum eval_status macro_expand_recursive(struct lisp_val ast,
                                               struct lisp_env *env,
                                               struct lisp_val *result) {
  struct lisp_val ast_expanded;
  enum eval_status res = macro_expand_full(ast, env, &ast_expanded);
  if (res != EV_SUCCESS) {
    return res;
  }

  if (should_stop_expansion(ast_expanded)) {
    *result = ast_expanded;
    return EV_SUCCESS;
  }

  // Use list_mapper to avoid creating a copy if there is no macro expansion
  struct list_mapper mapper;
  list_mapper_init(&mapper, ast_expanded);

  // Check for cons cell already happened
  while (!list_mapper_finished(&mapper)) {
    struct lisp_val expanded_node;
    res = macro_expand_recursive(list_mapper_get_next(&mapper), env,
                                 &expanded_node);
    if (res != EV_SUCCESS) {
      break;
    }

    list_mapper_append_next(&mapper, expanded_node);
  }

  *result = list_mapper_build(&mapper);
  return res;
}
...
enum eval_status top_level_eval(struct lisp_val ast, struct lisp_env *env,
                                struct lisp_val *result) {
  // TODO Figure out a better plan for when this expansion happens.
  // - If at top-level, then macros can only be defined at the top-level
  //   * Is special handling for (do) blocks a good-enough compromise?
  //   * Since inside a (do) or at top-level (essentially just a (do)) is the
  //     only place a macro could be defined and then used (I think?)
  // - If in functions, then nested functions will get re-expanded on execution
  //   of the outer function. Macro expansion still also needs to happen during
  //   regular evaluation at every step to account for top-level code
  //   * Maybe this is a sort of "best of both worlds" since the inner macros
  //     can be expanded with locally-defined macros, but won't need to
  //     repeatedly expand macros which were defined in the outer scope?
  //     Nested functions would still be getting re-scanned for no reason
  //     in most cases.

  struct lisp_val expanded_ast;
  // enum eval_status res = macro_expand_recursive(ast, env, &expanded_ast);
  // if (res == EV_EXCEPTION) {
  //   return res;
  // }
  expanded_ast = ast;

  return eval(expanded_ast, env, result);
}
