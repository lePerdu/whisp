...
struct lisp_env_binding *lisp_env_get(struct lisp_env *env,
  ...
  return (struct lisp_env_binding *)lisp_symbol_table_lookup_symbol(
      env->mappings, sym);
}
...
static struct lisp_env_binding *lisp_env_set_or_insert(struct lisp_env *env,
  ...
  struct lisp_env_binding *entry =
      (struct lisp_env_binding *)lisp_symbol_table_lookup_symbol(env->mappings,
                                                                 sym);
  if (entry != NULL) {
    entry->value = value;
  ...
  env->mappings = lisp_symbol_table_insert(
